package org.penpal.email.sending;
...
public class EmailThreadService {
    ...
    public ResponseEntity<?> getInactiveStudents() {
        ...
        Student[] allStudents = restTemplate.getForObject(AUTH_SERVICE_HOSTED_URL + "api/students", Student[].class);
    ...
    public ResponseEntity<?> remindInactiveStudents() {
        ...
        if (inactiveStudentsResponse != null && inactiveStudentsResponse.getBody() instanceof List<?>) {
            ...
            for (Student student : inactiveStudents) {
                String updateUrl = AUTH_SERVICE_HOSTED_URL + "api/students/" + student.getUserEmail() + "/account-status?status=" + AccountStatus.INACTIVE;
